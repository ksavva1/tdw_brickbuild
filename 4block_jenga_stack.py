...
class MagnebotLogCabin(Controller):
    ...
    def _go_home(self, bot: Magnebot):
        """Move arm to a safe home position clear of the table, then reset."""
        # Lift straight up to cruise altitude
        bot.reach_for(
            target={"x": self.STACK_X, "y": self.CRUISE_ALT, "z": self.STACK_Z},
            arm=Arm.right,
        )
        self._do(bot, "Retract: lift above stack")

        # Move laterally to home
        bot.reach_for(
            target={"x": self.HOME_X, "y": self.HOME_Y, "z": self.HOME_Z},
            arm=Arm.right,
        )
        self._do(bot, "Retract: move to home position")
    ...
    def _approach_from_above(self, bot: Magnebot, x: float, z: float,
                             target_y: float, label: str):
        """
        Two-step approach: first go to cruise altitude above (x, z),
        then lower straight down to target_y. This prevents the arm
        from sweeping through the table.
        """
        # Move to cruise altitude directly above the target
        bot.reach_for(
            target={"x": x, "y": self.CRUISE_ALT, "z": z},
            arm=Arm.right,
        )
        self._do(bot, f"{label}: move above ({x:.2f}, {z:.2f})")

        # Lower straight down
        bot.reach_for(
            target={"x": x, "y": target_y, "z": z},
            arm=Arm.right,
        )
        self._do(bot, f"{label}: lower to y={target_y:.2f}")
    ...
    def run(self):
        ...
        for i, bid in enumerate(block_ids):
            ...
            bot.slide_torso(height=self.GRASP_TORSO)
            self._do(bot, "Torso to 0.60 for grasp")

            # Approach block from above (prevents arm sweeping through the table)
            grasp_y = self.TABLE_SURFACE_Y + self.GRASP_DY

            # Reach high above the block from home position
            bot.reach_for(
                target={"x": bx, "y": self.CRUISE_ALT, "z": bz},
                arm=Arm.right,
            )
            self._do(bot, "Move above block")
            ...
            self._do(bot, "Lower to grasp height")
            ...
            # Place
            # Raise torso for higher layers
            place_torso = 0.6 + layer * 0.15
            if place_torso > self.GRASP_TORSO:
                bot.slide_torso(height=place_torso)
                self._do(bot, f"Raise torso to {place_torso:.2f} for placement")

            bot.reach_for(
                target={"x": place_x, "y": self.CRUISE_ALT, "z": place_z},
                arm=Arm.right,
            )
            ...
            bot.reach_for(
                target={"x": place_x, "y": drop_y, "z": place_z},
                arm=Arm.right,
            )
            ...
            self._settle(10)
            print(f"    ✔ Block {i} locked at ({place_x:.3f}, "
                  f"{place_y:.3f}, {place_z:.3f}), yaw={yaw}°")
